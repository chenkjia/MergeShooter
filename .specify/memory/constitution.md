...
- **渲染框架统一**: 项目统一使用 `Pixi.js` 作为渲染层，实现精灵、纹理、滤镜、九宫格与动效。禁止在业务代码中直接操作 `CanvasRenderingContext2D`。
- **平台适配**: 在抖音/微信小游戏环境下，通过适配层提供 `view`（如 `tt.createCanvas()`）交由 `Pixi.Application` 使用，保持渲染与平台 API 隔离。
- **资源加载**: 优先使用 `PIXI.Texture` 与相关加载能力统一管理图片资源，所有资源路径保持在 `src/assets` 下，加载后以纹理键值访问。
- **事件模型**: 交互统一采用 Pixi 交互系统或平台触摸事件转发，不得混用 DOM 事件。
...
- **图片资源管理**: 所有图片资源在使用前必须复制到 `/src/assets` 目录中，确保资源路径统一管理和正确加载。严禁直接引用外部路径的图片资源。
